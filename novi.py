...
class GraphVisualizer:
    def __init__(self):
        ...
        self.buttons = {
            ...
            "previous": pygame.Rect(200, 750, 80, 30),  # Changed from "step" to "previous"
            "next": pygame.Rect(290, 750, 80, 30),     # Added "next" button
            "reset": pygame.Rect(380, 750, 80, 30),    # Moved reset button
            "speed_down": pygame.Rect(470, 750, 30, 30),  # Adjusted positions
    ...
    def dijkstra_steps(self, graph, start):
        ...
        dist[start] = 0
        prev = {node: None for node in graph}  # Храним предыдущие узлы
        visited = set()
        ...
        self.prev = prev  # Сохраняем для дальнейшего отображения
        return steps
    ...
    def draw_arrowhead(self, start, end, color):
        ...
        node_radius = 20
        shrink = node_radius + 5  # Отступ от центра узла

        tip_x = end[0] - ux * shrink
        tip_y = end[1] - uy * shrink

        base_x = tip_x - ux * 10  # 10 пикселей назад от tip
        base_y = tip_y - uy * 10
